...
import { readLiveContinuationTriggers } from './repositories/live-continuation-triggers'
...
const worker: ExportedHandler<Bindings> = {
  async fetch(request, env, executionContext) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/status/continuation-triggers') {
      return Response.json(await readLiveContinuationTriggers(env.DB))
    }
