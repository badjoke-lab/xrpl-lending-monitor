...
// Catch-up must outrun Devnet ledger production while staying inside the D1 Free
// persistence envelope. Dense passes are already bounded independently by the
// persistence-window selector (32 ledgers / 24 coalesced mutations), so retaining
// the old four-hour successor delay only makes the Current cursor fall farther behind.
