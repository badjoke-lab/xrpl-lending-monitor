...
// Dense backlog must not be replayed through the Worker/D1 hot path at live-tail speed.
// Production evidence showed multi-thousand-row D1 writes per dense pass, so synthetic
// catch-up is intentionally reduced to the protected four-hour cadence while the
// fixed-target canonical extension path handles bulk recovery outside D1.
export const FAST_LANE_CATCH_UP_INTERVAL_MS = 4 * 60 * 60_000
