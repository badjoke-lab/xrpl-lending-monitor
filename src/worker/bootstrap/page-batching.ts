...
export async function persistPageBatches(options: {
  ...
  for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
    ...
    await writeBatch(options.db, {
      ...
      markerAfter: isLastChunk ? options.page.markerAfter : options.page.markerBefore,
      decodedObjectCount: isLastChunk ? options.page.decodedObjects : 0,
      vaults: chunk.vaults,
      loanBrokers: chunk.loanBrokers,
      loans: chunk.loans,
      cumulativeMetrics: options.metrics as unknown as Record<string, unknown>,
      writtenAt: updatedAt,
      advanceCheckpoint: isLastChunk,
    })
    nextSequence += 1
  }
