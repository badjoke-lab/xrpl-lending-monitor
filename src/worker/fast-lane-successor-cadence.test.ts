...
describe('fast-lane successor cadence', () => {
  it('uses a one-minute serial catch-up cadence', () => {
    let scheduledTime = Date.parse('2026-09-11T00:00:00Z')
    ...
    expect(FAST_LANE_CATCH_UP_INTERVAL_MS).toBe(60_000)
  })

  it('moves to the next one-minute boundary instead of replaying a stale schedule', () => {
    expect(nextFastLaneSuccessor({
      currentScheduledTime: Date.parse('2026-09-11T00:00:00Z'),
      now: Date.parse('2026-09-11T00:03:26Z'),
      caughtUp: false,
    })).toEqual({
      scheduledTime: Date.parse('2026-09-11T00:04:00Z'),
      cron: FAST_LANE_CATCH_UP_CRON,
    })
  })

  it('restores enough nominal catch-up throughput for the retained Devnet observation', () => {
    const ledgersPerPass = 32
    const passesPerFiveMinutes = (5 * 60_000) / FAST_LANE_CATCH_UP_INTERVAL_MS
    const nominalLedgersPerFiveMinutes = ledgersPerPass * passesPerFiveMinutes
    const retainedObservedLedgersPerFiveMinutes = 84

    expect(nominalLedgersPerFiveMinutes).toBe(160)
    expect(nominalLedgersPerFiveMinutes).toBeGreaterThan(retainedObservedLedgersPerFiveMinutes)
  })
