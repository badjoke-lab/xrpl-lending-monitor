...
describe('R4F G3 workflow safety boundary', () => {
  it('keeps the former remote probe restricted to the separately authorized steady reclaim repair path', () => {
    ...
    expect(reclaim).toContain("RECLAIM_GUARD_VERSION: '20260811012000'")
    expect(reclaim).toContain("MIGRATION_VERSION: '20260811061000'")
    expect(reclaim).toContain('MIGRATION_PATH: supabase/migrations/20260811061000_xrpl_r5_steady_reclaim_tick_accounting_fix.sql')
    expect(reclaim).toContain('supabase_migrations.schema_migrations')
    expect(reclaim).toContain('read_only:true')
    expect(reclaim).toContain("expected_tail='20260809151000 20260810123000 20260810133000 20260811012000 20260811061000'")
    expect(reclaim).toContain('Unexpected remote migration history for bounded reclaim repair')
    expect(reclaim).toContain('supabase db push --linked --dry-run')
    expect(reclaim).toContain('supabase db push --linked --yes')
    expect(reclaim.match(/supabase db push --linked --yes/g)).toHaveLength(1)
