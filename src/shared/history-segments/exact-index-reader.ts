...
export class HistoryExactIndexReader {
  ...
  async #bucket(bucket: number): Promise<{
    ...
    if (cached) {
      return { records: cached, assetReads: 0, compressedBytes: 0, decompressedBytes: 0 }
    }
    ...
    if (records.length !== asset.recordCount) {
      throw new Error('History exact index asset record count mismatch')
    }
    ...
    for (let index = 1; index < records.length; index += 1) {
      ...
      const order = previous.term.localeCompare(current.term)
        || previous.reference.ledgerIndex - current.reference.ledgerIndex
  ...
  async find(termValue: string, options: { limit?: number } = {}): Promise<HistoryExactLookupResult> {
