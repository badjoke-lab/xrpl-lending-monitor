...
async function fixture() {
  ...
  const records: HistoryExactIndexRecord[] = [
    ...
    { schemaVersion: 2, bucket, term: TERM.toUpperCase(), reference: { kind: 'object_change', segmentId: 's', fileKind: 'object_changes', ledgerIndex: 104, searchResult: { kind: 'object_change', epochId: 'e', ledgerIndex: 104, transactionHash: 'TX', objectType: 'Loan', objectId: 'L', loanId: 'L' } } },
  ]
...
describe('history exact index reader', () => {
  it('filters kinds before limit and caches the bucket', async () => { const f = await fixture(); const r = await HistoryExactIndexReader.open(f); const first = await r.find(TERM, { limit: 1, referenceKinds: ['object_change'] }); expect(first.references.map((x) => x.kind)).toEqual(['object_change']); expect(first.assetReads).toBe(1); expect((await r.find(TERM)).assetReads).toBe(0) })
