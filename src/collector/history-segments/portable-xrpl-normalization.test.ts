...
} from './portable-xrpl-normalization'
...
describe('portable XRPL seven-class normalization', () => {
  it('uses existing history derivations and emits all seven portable classes', async () => {
    ...
    expect(
      persisted.filter((row) => row.semanticClass === 'current-projection'),
    ).toEqual(
