...
function exactlyOne(rows, name) { if (rows.length !== 1) throw new Error(`${name} expected one row, found ${rows.length}`); return rows[0] }
function integer(value, name) { const parsed = Number(value); if (!Number.isSafeInteger(parsed) || parsed < 0) throw new Error(`${name} invalid`); return parsed }
...
const run = exactlyOne(await query(
  `select run_id, profile_id, profile_revision, profile_identity_digest, selection_digest,
     ...
     from xrpl_r5_v1.recovery_runs where run_id = $1::text`, [runId]
), 'R5 run')
if (run.run_id !== runId || run.profile_id !== 'supabase_free_postgres_pgcron_edge' || integer(run.profile_revision, 'profile revision') !== 4 || run.profile_identity_digest !== profileDigest || run.source_profile_id !== 'supabase-devnet' || run.network !== 'devnet' || run.epoch_id !== 'supabase-r4c2c-v1' || typeof run.base_identity !== 'string' || run.base_identity.length === 0) throw new Error('R5 run identity is not the exact revision-4 Devnet run')

const batchCounts = exactlyOne(await query(
  `select count(*)::bigint as total,
          count(*) filter (where status = 'leased')::bigint as leased,
          count(*) filter (where status = 'completed')::bigint as committed,
          count(*) filter (where status = 'halted')::bigint as failed
     from xrpl_r5_v1.recovery_batches where run_id = $1::text`, [runId]
), 'R5 batch counts')
const latestBatchRows = await query(`select to_jsonb(b) as batch from xrpl_r5_v1.recovery_batches b where b.run_id = $1::text order by b.batch_sequence desc, b.batch_id desc limit 1`, [runId])
...
const watermark = exactlyOne(await query(`select profile_id, network, epoch_id, base_identity, ledger_index, ledger_hash, work_id, updated_at from public.xrpl_phase_watermarks where profile_id = 'supabase-devnet'`), 'public watermark')
if (watermark.network !== run.network || watermark.epoch_id !== run.epoch_id || watermark.base_identity !== run.base_identity) throw new Error('public watermark identity does not match retained R5 run')
...
if (boundaryDriftLedgers === 0 && (watermark.ledger_hash !== run.current_watermark_ledger_hash || watermark.work_id !== run.current_watermark_work_id)) throw new Error('same-ledger public watermark identity conflicts with retained R5 boundary')
...
const snapshot = exactlyOne(await query(
  `select snapshot_id, source_run_id, source_commit, observed_at,
     ...
     from xrpl_resource_guard_v1.external_snapshots order by observed_at desc, snapshot_id desc limit 1`
...
if (snapshot.management_api_available !== true || functionCount < 1 || maxBundleBytes < 1 || bundleCount < 1 || typeof snapshot.max_bundle_name !== 'string' || snapshot.max_bundle_name.length === 0 || typeof snapshot.evidence_digest !== 'string' || !/^[a-f0-9]{64}$/u.test(snapshot.evidence_digest)) throw new Error('fresh resource snapshot lacks required exact management/bundle coverage')

const migrationRows = await query(`select version::text as version from supabase_migrations.schema_migrations where version::text in ($1::text, $2::text) order by version::text`, [baseMigrationVersion, targetMigrationVersion])
...
const migrationMax = String(exactlyOne(await query(`select max(version::text) as max_version from supabase_migrations.schema_migrations`), 'migration max').max_version ?? '')
...
if (targetApplied && !approvedCurrentMigrationVersions.has(migrationMax)) throw new Error(`unreviewed migration exists after minute follow-up:${migrationMax}`)
...
const contract = exactlyOne(await query(
  `select p.prosecdef as security_definer, pg_get_functiondef(p.oid) as function_definition,
          has_function_privilege('anon', p.oid, 'EXECUTE') as anon_execute,
          has_function_privilege('authenticated', p.oid, 'EXECUTE') as authenticated_execute,
          has_function_privilege('service_role', p.oid, 'EXECUTE') as service_role_execute
     from pg_proc p where p.oid = to_regprocedure($1::text)`, [continuousSignature]
...
if (run.status === 'halted' && run.last_error === 'r5_recovery_monthly_invocation_halt') {
  if (totalBatches !== 0 || committedBatches !== 0 || runCompletedBatches !== 0 || committedLedgers !== 0 || run.last_accounting_digest !== null) throw new Error('invocation-halted R5 run contains unexpected recovery progress')
  activationMode = 'halted_invocation_rearm'
} else if (run.status === 'prepared') {
  if (totalBatches !== 0 || runCompletedBatches !== 0 || committedLedgers !== 0 || run.last_accounting_digest !== null) throw new Error('prepared R5 run contains unexpected recovery progress')
  activationMode = 'prepared_continue'
} else if (run.status === 'running') {
  if (run.last_error !== null || failedBatches !== 0 || runCompletedBatches < 1 || committedLedgers < 1 || typeof run.last_accounting_digest !== 'string' || !/^[a-f0-9]{64}$/u.test(run.last_accounting_digest)) throw new Error('running R5 run does not have a clean committed continuation state')
...
const stableBinding = {
  ...
  profileRevision: Number(run.profile_revision), profileIdentityDigest: run.profile_identity_digest, selectionDigest: run.selection_digest,
  ...
  boundaryDriftLedgers, totalBatches, failedBatches, completedBatches: runCompletedBatches, committedLedgers,
  snapshotId: snapshot.snapshot_id, snapshotObservedAt: snapshot.observed_at, snapshotSourceRunId: Number(snapshot.source_run_id), snapshotSourceCommit: snapshot.source_commit,
  invocationCount24h, projectedInvocations31d, invocationHalt31d, functionCount, maxBundleBytes, maxBundleName: snapshot.max_bundle_name, bundleCount,
  baseMigrationVersion, targetMigrationVersion, migrationState, currentMaxMigrationVersion: migrationMax, currentMaxVersionApproved, functionDefinitionDigest,
}
...
const evidence = {
  ...
  run: {
    ...
    profileRevision: Number(run.profile_revision), profileIdentityDigest: run.profile_identity_digest, selectionDigest: run.selection_digest,
    currentWatermarkLedgerIndex: runWatermarkLedger, currentWatermarkLedgerHash: run.current_watermark_ledger_hash,
    initialValidatedHeadLedgerIndex: integer(run.initial_validated_head_ledger_index, 'initial head ledger'),
    completedBatches: runCompletedBatches, committedLedgers, lastAccountingDigest: run.last_accounting_digest,
  },
  ...
  batchCounts: { total: totalBatches, leased: leasedBatches, committed: committedBatches, failed: failedBatches },
  latestBatch,
  resourceSnapshot: {
    snapshotId: snapshot.snapshot_id, sourceRunId: Number(snapshot.source_run_id), sourceCommit: snapshot.source_commit,
    observedAt: snapshot.snapshot_observed_at ?? snapshot.observed_at, fresh: snapshotFresh,
    invocationCount24h, projectedInvocations31d, invocationHalt31d,
    functionCount, maxBundleBytes, maxBundleName: snapshot.max_bundle_name, bundleCount, evidenceDigest: snapshot.evidence_digest,
  },
  migration: {
    baseMigrationVersion, targetMigrationVersion, targetApplied, migrationState,
    currentMaxVersion: migrationMax, currentMaxVersionApproved, approvedCurrentMigrationVersions: [...approvedCurrentMigrationVersions], functionDefinitionDigest,
  },
  stateDigest,
