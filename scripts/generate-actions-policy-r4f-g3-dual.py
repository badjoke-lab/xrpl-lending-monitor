...
if len(sys.argv) != 3:
    raise SystemExit('usage: generate-actions-policy-r4f-g3-dual.py <source> <output>')
...
for required in (
    ...
    "startsWith(github.event.comment.body, '/r4f-g3-prepare ')",
    "verify-r4f-g3-before-sequence.mjs",
    "startsWith(github.event.comment.body, '/r4f-g3-authorize ')",
    "dashboard_auth=([0-9]+)",
    "pause_run=([0-9]+)",
    "before_comment=([0-9]+)",
    "startsWith(github.event.comment.body, '/r4f-g3-after ')",
...
for required in (
    ...
    "verify-r4f-g3-resume-sequence.mjs",
...
g3_dual_text = (root / g3_dual).read_text()
for required in (
    "contents: read",
    "actions: read",
    "issues: write",
    ...
    "github.event.issue.number == 1261",
    "github.event.comment.user.login == 'badjoke-lab'",
    "startsWith(github.event.comment.body, '/r4f-g3-finalize ')",
    "verify-r4f-g3-after-sequence.mjs",
    ...
    "r4f-g3-formal-dual-verdict",
    "profileSelected: false",
    "r5Authorized: false",
    "mainnetDisabled: true",
    "Require exact dual qualification",
):
    if required not in g3_dual_text:
        raise SystemExit(f"R4F G3 dual-verdict workflow is missing fail-closed requirement: {required}")
for forbidden in (
    "  push:",
    "  schedule:",
    "pull_request_target",
    ...
    if forbidden in g3_dual_text:
        raise SystemExit(f"R4F G3 dual-verdict workflow contains forbidden capability: {forbidden.strip()}")
if g3_dual_text.count("issues: write") != 1:
    raise SystemExit("R4F G3 dual-verdict issue-write capability must remain exactly one permission")
'''
