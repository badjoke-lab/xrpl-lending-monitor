...
function validateBefore(state){ validateCommon(state); if(Number(state.databaseBytes)>MAX_DATABASE_BYTES_BEFORE) fail('database above ready-index reindex ceiling'); if(Number(state.indexBytes)<=0||Number(state.indexBytes)>MAX_INDEX_BYTES_BEFORE) fail('ready index bytes outside authorized ceiling'); if(Number(state.readyRows)<0||Number(state.readyRows)>MAX_READY_ROWS) fail('ready row count outside authorized ceiling'); if(Number(state.tableRows)<=0) fail('phase message table unexpectedly empty') }
function validateAfter(state){ validateCommon(state); if(Number(state.readyRows)>MAX_READY_ROWS) fail('ready row count outside post-reindex ceiling') }
function structuralState(state,sourceCommit){ return {schemaVersion:1,purpose:'r5-phase-ready-index-physical-reindex-structural-state',sourceCommit,projectIdentityDigest:sha256(env('SUPABASE_PROJECT_ID')),maxMigrationVersion:state.maxMigrationVersion,tableOid:Number(state.tableOid),indexOid:Number(state.indexOid),indexDefinition:state.indexDefinition,indexDefinitionSha256:state.indexDefinitionSha256,indexValid:state.indexValid,indexReady:state.indexReady,indexUnique:state.indexUnique,indexPrimary:state.indexPrimary,activeRun:state.activeRun,scheduler:state.scheduler} }
function dataState(state){ return {schemaVersion:1,purpose:'r5-phase-ready-index-physical-reindex-data-state',databaseBytes:Number(state.databaseBytes),tableHeapBytes:Number(state.tableHeapBytes),tableRows:Number(state.tableRows),readyRows:Number(state.readyRows),statusCounts:state.statusCounts,indexBytes:Number(state.indexBytes),indexScans:Number(state.indexScans)} }
function mutationSql(expected){
  for(const key of ['tableRows','readyRows','tableHeapBytes','indexBytes']) if(!Number.isSafeInteger(Number(expected[key]))||Number(expected[key])<0) fail(`invalid mutation ${key}`)
...
begin
  ...
  if current_definition<>${JSON.stringify(EXPECTED_INDEX_DEFINITION)} then raise exception 'ready index definition drift under lock'; end if;
...
for(const required of ["lock table public.xrpl_phase_messages in share mode","ready index authorized data drift under lock","ready index reindex safety ceiling exceeded under lock","reindex index public.xrpl_phase_messages_ready_idx"]) if(!SAMPLE.includes(required)) fail(`ready reindex mutation missing: ${required}`)
for(const forbidden of [/\bdelete\s+from\b/iu,/\btruncate\b/iu,/\bupdate\s+public\b/iu,/\binsert\s+into\s+public\b/iu,/\bvacuum\b/iu,/\bcluster\b/iu,/\bcron\.schedule\b/iu,/\bcron\.unschedule\b/iu,/\bmainnet\b/iu]) if(forbidden.test(SAMPLE)) fail(`ready reindex mutation contains forbidden capability: ${forbidden}`)

async function inspect(sourceCommit,after=false){
  const state=firstJson(await query(INSPECTION_SQL,true)); if(after) validateAfter(state); else validateBefore(state)
  const structural=structuralState(state,sourceCommit),data=dataState(state),structuralSha=sha256(JSON.stringify(structural)),dataSha=sha256(JSON.stringify(data)),mutation=mutationSql(data)
  const plan={schemaVersion:1,purpose:'r5-phase-ready-index-physical-reindex-plan',sourceCommit,structuralStateSha256:structuralSha,dataStateSha256:dataSha,mutationSha256:sha256(mutation),maxDatabaseBytesBefore:MAX_DATABASE_BYTES_BEFORE,maxIndexBytesBefore:MAX_INDEX_BYTES_BEFORE,maxReadyRows:MAX_READY_ROWS,lockMode:'SHARE',lockRevalidation:true}
  return {...state,structuralState:structural,structuralStateSha256:structuralSha,dataState:data,dataStateSha256:dataSha,mutation:{sha256:sha256(mutation),lockTimeoutSeconds:5,statementTimeoutSeconds:45,lockRevalidation:true},plan,planSha256:sha256(JSON.stringify(plan)),rowMutationAuthorized:false,vacuumAuthorized:false,schedulerMutationAuthorized:false,deploymentAuthorized:false,publicReaderMutationAuthorized:false,mainnetDisabled:true,r5RearmAuthorized:false}
}
async function prepare(options){ const sourceCommit=validateSource(options),result=await inspect(sourceCommit,false); await writeJson(options.output,result); console.log(JSON.stringify(result)) }
async function apply(options){
  ...
  const mutation=mutationSql(before.dataState); if(sha256(mutation)!==authorizedMutation) fail('authorized mutation mismatch')
  ...
  const result={schemaVersion:1,purpose:'r5-phase-ready-index-physical-reindex-apply',sourceCommit,structuralStateSha256:authorizedState,dataStateSha256:authorizedData,planSha256:authorizedPlan,mutationSha256:authorizedMutation,tableRowsBefore:Number(before.tableRows),tableRowsAfter:Number(after.tableRows),readyRowsBefore:Number(before.readyRows),readyRowsAfter:Number(after.readyRows),tableHeapBytesBefore:Number(before.tableHeapBytes),tableHeapBytesAfter:Number(after.tableHeapBytes),indexBytesBefore:Number(before.indexBytes),indexBytesAfter:Number(after.indexBytes),indexBytesReclaimed:Number(before.indexBytes)-Number(after.indexBytes),databaseBytesBefore:Number(before.databaseBytes),databaseBytesAfter:Number(after.databaseBytes),databaseBytesReclaimed:Number(before.databaseBytes)-Number(after.databaseBytes),indexOidPreserved:Number(after.indexOid)===Number(before.indexOid),indexDefinitionPreserved:after.indexDefinition===before.indexDefinition,activeRunUnchanged:JSON.stringify(after.activeRun)===JSON.stringify(before.activeRun),schedulerUnchanged:JSON.stringify(after.scheduler)===JSON.stringify(before.scheduler),rowMutationPerformed:false,vacuumPerformed:false,schedulerMutationPerformed:false,deploymentPerformed:false,publicReaderMutationPerformed:false,mainnetDisabled:true,r5RearmPerformed:false}
