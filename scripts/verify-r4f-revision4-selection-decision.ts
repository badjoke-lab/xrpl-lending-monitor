...
} from '../src/shared/supabase-revision4-selection-decision'
...
function usage(): string {
  return [
    'Usage:',
    '  node r4f-revision4-selection-decision-verifier.mjs --input <path> [--output <path>] [--require-proof-ready]',
    '',
    'Offline verifier only. It does not select a profile or authorize R5 mutation.',
  ].join('\n')
}
...
function main(): void {
  ...
  const input = JSON.parse(readFileSync(resolve(args.inputPath), 'utf8')) as SupabaseRevision4SelectionDecisionInput
  ...
  console.log(JSON.stringify({ evidenceId: result.evidenceId, evidenceClass: result.evidenceClass, proofReady: result.proofReady, decisionReady: result.decisionReady, blockingReasons: result.blockingReasons, machineSummary: result.machineSummary, outputPath: args.outputPath ? resolve(args.outputPath) : null }))
