...
async function splitRange(options: Arguments, sourceManifest: HistoryExactIndexManifest): Promise<void> {
  ...
  for (let sourceBucket = start; sourceBucket < end; sourceBucket += 1) {
    ...
    const lower: HistoryExactIndexRecord[] = []
    const upper: HistoryExactIndexRecord[] = []
    for (const raw of decoded.records) {
      ...
      if (targetBucket !== sourceBucket && targetBucket !== sourceBucket + SOURCE_BUCKET_COUNT) {
        throw new Error(`Target bucket is not a valid power-of-two refinement: ${sourceBucket}:${targetBucket}`)
      }
      ...
      ;(targetBucket === sourceBucket ? lower : upper).push(targetRecord)
    }
    assertSorted(lower, `Target bucket ${sourceBucket}`)
    assertSorted(upper, `Target bucket ${sourceBucket + SOURCE_BUCKET_COUNT}`)
    const lowerAsset = await writeTargetAsset({
    ...
    const upperAsset = await writeTargetAsset({
      outputDir: options.outputDir,
      assetPrefix: options.assetPrefix,
      sourceBucket,
      targetBucket: sourceBucket + SOURCE_BUCKET_COUNT,
      records: upper,
    })
    assets.push(lowerAsset, upperAsset)
    sourceRecords += descriptor.recordCount
    outputRecords += lower.length + upper.length
    maxDecompressedBytes = Math.max(
      maxDecompressedBytes,
      lowerAsset.decompressedBytes,
      upperAsset.decompressedBytes,
    )
    process.stderr.write(
      `[rebucket] source ${sourceBucket} done lower=${lower.length} upper=${upper.length} max=${Math.max(lowerAsset.decompressedBytes, upperAsset.decompressedBytes)}\n`,
    )
  }
