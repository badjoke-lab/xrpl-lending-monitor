...
const maximumWatermark = minimumWatermark + maximumWatermarkAdvance
...
with run_state as (
  ...
  select coalesce(jsonb_agg(to_jsonb(message) order by message.message_id), '[]'::jsonb) as value
...
select jsonb_build_object(
  'purpose', 'r5-pending-scan-read-only-diagnostic-v2',
...
function excerpt(definition) {
  ...
  return definition.slice(
    ...
    Math.min(definition.length, index + exactError.length + 5_000),
  )
}

await mkdir(output, { recursive: true })
let retainedEvidence = false

try {
  const stateRows = await query(stateSql, [
    recoveryRunId,
    sourceRunId,
    sourceCommit,
  ])
  if (stateRows.length !== 1) {
    throw new Error(`unexpected state row count ${stateRows.length}`)
  }
  ...
  const noncommittedWork = list(state.noncommittedWork, 'noncommittedWork')
  const recentWork = list(state.recentWork, 'recentWork')
  const nonterminalMessages = list(
    state.nonterminalMessages,
    'nonterminalMessages',
  )
  const activeBatches = list(state.activeBatches, 'activeBatches')
  const recentBatches = list(state.recentBatches, 'recentBatches')
  const recoveryWatermark = integer(
    run.current_watermark_ledger_index,
    'run watermark',
  )
  const physicalWatermark = integer(
    watermark.ledger_index,
    'physical watermark',
  )
  ...
  const checks = {
    ...
    recoveryWatermarkAtLeastMinimum: recoveryWatermark >= minimumWatermark,
    recoveryWatermarkWithinDiagnosticBound:
      recoveryWatermark <= maximumWatermark,
    exactErrorLocatedOnce: exactFunctions.length === 1,
    noActiveBatch: activeBatches.length === 0,
  ...
  const evidence = {
    ...
    recoveryWatermark,
    physicalWatermark,
    physicalAheadBy: physicalWatermark - recoveryWatermark,
    minimumWatermark,
    maximumWatermarkAdvance,
    maximumWatermark,
    checks,
    checkFailures,
  }
  ...
  const exact = exactFunctions[0] ?? functions[0]
  const markdown = [
    '## R5 pending-scan read-only diagnostic V2',
    '',
    `- run: ${code(sourceRunId)}`,
    `- commit: ${code(sourceCommit)}`,
    `- diagnostic: ${code(checkFailures.length === 0 ? 'success' : 'boundary-failure')}`,
    `- R5 watermark ledger: ${code(recoveryWatermark)}`,
    `- accepted R5 watermark range: ${code(`${minimumWatermark}-${maximumWatermark}`)}`,
    `- R5 watermark work: ${code(run.current_watermark_work_id)}`,
    `- physical watermark ledger: ${code(physicalWatermark)}`,
    `- physical watermark work: ${code(watermark.work_id)}`,
    `- physical ledgers ahead: ${code(physicalWatermark - recoveryWatermark)}`,
    `- noncommitted work rows: ${code(noncommittedWork.length)}`,
    `- nonterminal message rows: ${code(nonterminalMessages.length)}`,
    `- active R5 batches: ${code(activeBatches.length)}`,
    `- exact error function count: ${code(exactFunctions.length)}`,
    `- exact error function: ${code(exact?.signature)}`,
    `- function definition SHA-256: ${code(exact?.definitionSha256)}`,
    `- failed checks: ${code(checkFailures.length === 0 ? 'none' : checkFailures.join(','))}`,
    ...
    '',
    '```sql',
    exact?.excerpt ?? 'exact function not found',
    '```',
    '',
  ].join('\n')
  ...
  if (checkFailures.length > 0) {
    throw new Error(`diagnostic checks failed: ${checkFailures.join(',')}`)
  }
} catch (error) {
  if (!retainedEvidence) {
