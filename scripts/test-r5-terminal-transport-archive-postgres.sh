...
-- Exact terminal identity is retained in heap; only fixed-width digests are indexed.
...
begin
  select * into a from proof.terminal_archive where message_hash=h;
  if found then
...
declare
  ...
  current_hash bytea := extensions.digest(convert_to(p_current,'UTF8'),'sha256');
  successor_hash bytea := extensions.digest(convert_to(p_successor,'UTF8'),'sha256');
begin
  select * into a from proof.terminal_archive where message_hash=current_hash;
  if found then
  ...
  select * into collision from proof.terminal_archive where successor_hash=successor_hash;
...
-- 6k terminal messages plus 2k retained messages. IDs and JSON are deliberately wide.
with generated as (
  select g,
...
select id,'supabase-devnet','scan',payload,
  case when g<=6000 then 'completed' else case when g=8001 then 'pending' else 'completed' end,
...
insert into proof.successors(current_message_id,successor_message_id,reserved_at)
select m.message_id,m.successor_message_id,coalesce(m.completed_at,'2026-08-16 12:00:00+00')
from proof.messages m
where m.successor_message_id is not null;

-- Force one old-current -> retained-successor cutoff edge, matching production shape.
update proof.messages set successor_message_id=(select message_id from proof.messages where message_id like 'scan:v1:devnet:epoch:6001:%')
where message_id like 'scan:v1:devnet:epoch:6000:%';
update proof.successors set successor_message_id=(select message_id from proof.messages where message_id like 'scan:v1:devnet:epoch:6001:%')
where current_message_id like 'scan:v1:devnet:epoch:6000:%';
...
-- Archive now owns the exact mapping, so only edges whose current was archived are removed.
delete from proof.successors s
using proof.terminal_archive a
where s.current_message_id=a.message_id;

-- Fail closed if a retained current still points to a message selected for deletion.
do $$ begin
  if exists(
    select 1 from proof.successors s join proof.terminal_archive a on a.message_id=s.successor_message_id
  ) then raise exception 'retained current points to archived successor'; end if;
end $$;

delete from proof.messages m
using proof.terminal_archive a
...
# Same archived identity converges without recreating a live row.
...
# Payload drift for an archived ID must fail closed.
set +e
docker exec "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres -Atqc \
...
payload_conflict_rc=$?
set -e
[[ "$payload_conflict_rc" -ne 0 ]]
grep -q 'phase message identity conflict' "${output_directory}/payload-conflict.stderr"

# Same successor mapping converges from archive; a changed mapping fails closed.
docker exec "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres -Atqc \
...
set -e
[[ "$successor_conflict_rc" -ne 0 ]]
grep -q 'phase successor identity conflict' "${output_directory}/successor-conflict.stderr"

# Archived completion remains a duplicate and returns the exact successor ID.
...
- full message ID preserved in archive: \`true\`
- full payload JSON preserved in archive: \`true\`
- exact successor ID preserved in archive: \`true\`
...
This proves only the compact terminal identity semantics and local physical-shrink direction. It does not prove compatibility with every historical recovery/qualification consumer and does not authorize production DELETE, table rewrite, VACUUM FULL, reindex, or R5 rearm.
EOF
