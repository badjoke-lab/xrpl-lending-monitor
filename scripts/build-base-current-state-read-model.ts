...
function parseArguments(args: readonly string[]): Arguments {
  if (!args.includes('--local')) throw new Error('Base current-state read-model generation requires --local')
  return {
...
async function main(): Promise<void> {
  ...
  if (!sourceManifest.complete) throw new Error('Base read model requires a complete source snapshot')

  const snapshotId = flatText(args.snapshotId ?? sourceManifest.snapshotId, 'snapshotId')
  const releaseTag = flatText(args.releaseTag ?? sourceManifest.releaseTag, 'releaseTag')
  ...
  for (const asset of sourceManifest.dataAssets) {
    ...
    ingested += await ingestDataAsset(db, join(args.inputDir, asset.assetName), asset.recordCount, counts)
  }
  if (ingested !== sourceManifest.relevantObjectCount) throw new Error('Total base ingested object count mismatch')
  if (canonicalJson(counts) !== canonicalJson(sourceManifest.counts)) throw new Error('Base object type counts mismatch')
  ...
  await writeFile(join(args.outputDir, 'base-read-model-summary.json'), `${canonicalJson(summary)}\n`, 'utf8')
  db.close()
  await rm(workDir, { recursive: true, force: true })
