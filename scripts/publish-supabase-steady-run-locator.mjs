...
if (success) {
  ...
  lines.push(
    ...
    `- steady total-memory RSS counter available: \`${String(memoryCapability?.usableTotalMemoryCounter ?? false)}\``,
    `- steady partial heap counters available: \`${String(memoryCapability?.partialHeapCountersAvailable ?? false)}\``,
    `- all RSS counters zero: \`${String(memoryCapability?.allRssCountersZero ?? 'unknown')}\``,
    `- all heap-total counters zero: \`${String(memoryCapability?.allHeapTotalCountersZero ?? 'unknown')}\``,
    `- all runtime memory counters zero: \`${String(memoryCapability?.allRuntimeCountersZero ?? 'unknown')}\``,
    `- partial heap counters substituted for RSS: \`${String(!(memoryCapability?.checks?.partialHeapCountersNotSubstitutedForRss ?? false))}\``,
    `- zero RSS interpreted as zero usage: \`${String(!(memoryCapability?.checks?.zeroRssNotInterpretedAsZeroUsage ?? false))}\``,
