...
async function loadSql(expectedSha) {
  ...
  for (const forbidden of [/\btruncate\b/iu,/\bdelete\s+from\b/iu,/\bupdate\s+(?:public\.|xrpl_r5_v1\.)?(?:xrpl_phase_|recovery_runs|recovery_batches)/iu,/\bvacuum\b/iu,/\bdrop\s+(?:table|schema)\b/iu,/\bcron\.schedule\b/iu,/\bcron\.unschedule\b/iu]) if (forbidden.test(sql)) fail(`staged SQL contains forbidden mutation: ${forbidden}`)
  return { sql, actualSha }
}
function inspectionQuery() {
  return `with defs as (
    select pg_get_functiondef('public.xrpl_claim_r5_revision4_recovery_batch_from_prepared_head(text,text,timestamp with time zone,integer)'::regprocedure) rev4,
           pg_get_functiondef('public.xrpl_claim_r5_active_recovery_batch_from_prepared_head(text,text,timestamp with time zone,integer)'::regprocedure) rev3
  ), run_state as (
    select to_jsonb(r) value from xrpl_r5_v1.recovery_runs r where r.run_id='${RUN_ID}'
  ), job_state as (
    select coalesce(jsonb_agg(jsonb_build_object('jobid',jobid,'jobname',jobname,'schedule',schedule,'command',command,'active',active) order by jobid),'[]'::jsonb) value
  ...
  select jsonb_build_object(
    ...
    'revision4Guards',(select jsonb_build_object(
      ...
      'noScanExecutedBeforeClaim',position('noScanExecutedBeforeClaim' in rev4)>0) from defs),
...
function parseState(rows) {
  ...
  if (!raw) fail('inspection returned no rows')
  return typeof raw === 'string' ? JSON.parse(raw) : raw
}
function expectedHistoryRecord(sqlSha) { return { version: VERSION, statements: [`exact-r5-revision4-prepared-head-memory-retry-fix sha256:${sqlSha}`], name: NAME } }
...
function allRevision4GuardsTrue(guards) { return Boolean(guards?.profileRevision4===true && guards?.revision4Rebind===true && guards?.revision4Claim===true && guards?.revision4Adoption===true && guards?.pendingScanLockHeldThroughClaim===true && guards?.noScanExecutedBeforeClaim===true) }
function schedulerIsRestoredCollector(scheduler) { return Array.isArray(scheduler) && scheduler.length===1 && scheduler[0]?.jobname===JOB_NAME && scheduler[0]?.schedule==='* * * * *' && scheduler[0]?.active===true && String(scheduler[0]?.command??'').includes('xrpl-collector-tick') && !String(scheduler[0]?.command??'').includes('xrpl-r5-minute-driver') }
function runIsZeroProgressPrepared(run) { return Boolean(run && run.run_id===RUN_ID && run.status==='prepared' && Number(run.profile_revision)===4 && Number(run.completed_batches)===0 && Number(run.committed_ledgers)===0 && run.last_error==null && run.last_accounting_digest==null && run.started_at==null && run.completed_at==null) }
function runIsProgressedRunning(run) { return Boolean(run && run.run_id===RUN_ID && run.status==='running' && Number(run.profile_revision)===4 && Number(run.completed_batches)>=1 && Number(run.committed_ledgers)>=1 && Number(run.current_watermark_ledger_index)>0 && /^[a-f0-9]{64}$/iu.test(String(run.last_accounting_digest??'')) && run.last_error==null && run.started_at!=null && run.completed_at==null) }
function runIsRepairable(run) { return runIsZeroProgressPrepared(run) || runIsProgressedRunning(run) }
function classify(state, sqlSha, sourceCommit) {
  ...
  const runLedger=Number(state.run?.current_watermark_ledger_index)
  const preCommon=allRevision4GuardsTrue(state.revision4Guards) && Number(state.revision3MemoryRetryCalls)===1 && runIsRepairable(state.run) && Number(state.activeBatchCount)===0 && schedulerIsRestoredCollector(state.scheduler) && Number.isSafeInteger(canonicalLedger) && Number.isSafeInteger(runLedger) && canonicalLedger>=runLedger
...
async function apply(options) {
  const sourceCommit=options['source-commit']; const expectedSha=options['expected-sha']; const expectedState=options['expected-state']; const output=options.output
  if (!/^[a-f0-9]{40}$/u.test(sourceCommit??'')) fail('source commit invalid')
