...
async function loadBaseBuckets(options: {
  ...
  for (const asset of options.manifest.assets) {
    ...
    for (const record of records) {
      assertHistoryExactIndexRecord(record, options.manifest.bucketCount)
      if (record.bucket !== asset.bucket) throw new Error(`Base exact-index bucket mismatch: ${asset.path}`)
    }
...
async function main(): Promise<void> {
  ...
  for (let bucket = 0; bucket < baseManifest.bucketCount; bucket += 1) {
    ...
    sortRecords(records)
    for (const record of records) assertHistoryExactIndexRecord(record, baseManifest.bucketCount)
