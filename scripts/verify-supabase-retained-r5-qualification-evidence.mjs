...
async function queryRetainedState() {
  ...
  if (!/^[a-z]{20}$/.test(projectRef)) throw new Error('SUPABASE_PROJECT_ID must be an exact 20-character project ref')
  const accessToken = process.env.SUPABASE_ACCESS_TOKEN ?? ''
  if (accessToken.length < 20) throw new Error('SUPABASE_ACCESS_TOKEN is unavailable for retained evidence verification')
...
select jsonb_build_object(
  ...
  'archive', (
    select to_jsonb(archive) || jsonb_build_object(
      'digestValid', public.xrpl_transfer_json_digest(archive.evidence) = archive.evidence_digest
    )
...
function verifyState(state) {
  ...
  if (databaseBytes >= databaseHaltBytes) throw new Error(`database remains above the fixed halt: ${databaseBytes}`)
  if (integer(state.catchupLiveRows, 'catch-up live rows') !== 0) throw new Error('catch-up raw qualification rows were not fully reclaimed')
  if (integer(state.catchupTotalBytes, 'catch-up total bytes') >= 17_000_000) throw new Error('catch-up qualification storage was not materially reclaimed')
  if (
    ...
    throw new Error('catch-up archive identity or digest is invalid')
  }
  if (
  ...
  try {
    ...
    const combined = {
      ...
      checks: {
        ...
        catchUpEvidenceDigestValid: true,
...
if (process.argv[1]?.endsWith('verify-supabase-retained-r5-qualification-evidence.mjs')) {
  ...
  if (retained === null) throw new Error('active R5 recovery ownership is required for retained qualification verification')
