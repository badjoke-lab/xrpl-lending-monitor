...
const activityResponse = {
  ...
  data: [
    {
      transaction_hash: firstHash, epoch_id: 'epoch-1', ledger_index: 123, event_index: 1,
...
async function mockApis(page: Page) {
  ...
  await page.route('**/api/activity?limit=100', (route) => route.fulfill({ json: activityResponse }))
}

test('renders and filters the bounded Activity window', async ({ page }) => {
  await mockApis(page)
  ...
  await expect(page.locator('.activity-card').first().locator('a.activity-hash')).toHaveAttribute(
    'href',
    `/transactions/${firstHash}`,
  )
  ...
  await expect(page.locator('.activity-card')).toContainText('Loanset')
