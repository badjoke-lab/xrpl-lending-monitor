...
begin
  ...
  if (select encode(extensions.digest(convert_to(p.prosrc, 'UTF8'), 'sha256'), 'hex')
    ...
    raise exception 'legacy_rev3_retirement_checkpoint_source_drift';
  end if;
...
revoke all privileges on function public.xrpl_create_r5_active_checkpoint_strict(text,timestamp with time zone)
