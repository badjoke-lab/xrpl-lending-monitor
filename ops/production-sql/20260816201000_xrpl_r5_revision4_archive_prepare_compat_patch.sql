...
begin
  ...
  if v_sha <> v_expected_before then raise exception 'revision4 archive prepare source drift: %', v_sha; end if;
  ...
  select pg_get_functiondef(v_signature::regprocedure) into v_definition;
  v_sha := encode(extensions.digest(convert_to(v_definition, 'UTF8'), 'sha256'), 'hex');
  if v_sha <> v_expected_after then raise exception 'revision4 archive prepare post-apply digest mismatch: %', v_sha; end if;
end;
$patch$;
