...
begin
  ...
  if not found
    ...
    or v_accounting.recorded_at > statement_timestamp()
    or v_accounting.created_at > statement_timestamp()
...
-- The accounting RPC commits in a separate transaction before the completion RPC.
-- Server-side statement time therefore proves the required ordering without trusting
-- a client-generated completion timestamp that may have been prepared milliseconds earlier.
