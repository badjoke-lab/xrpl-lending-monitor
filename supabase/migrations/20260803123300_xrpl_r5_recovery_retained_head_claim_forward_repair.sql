-- Migration 20260803123200 is present in the remote migration history, but its
-- statement-0 CREATE OR REPLACE failed against an orphaned exact signature.
-- Repair forward only: remove that exact function and recreate the canonical
-- implementation and grants after the current remote migration head.
