create or replace function public.xrpl_repair_r5_halted_batch_after_boundary_drain(
  p_run_id text,
  p_batch_id text,
...
begin
  if p_run_id <> v_expected_run_id
    ...
    raise exception 'r5_halted_boundary_repair_invalid_request';
  end if;
  ...
  if not found
    ...
    raise exception 'r5_halted_boundary_repair_run_invalid';
  end if;
  ...
  if not found
    ...
    raise exception 'r5_halted_boundary_repair_batch_invalid';
  end if;
  ...
  if v_completed_batch_count <> 86
    ...
    raise exception 'r5_halted_boundary_repair_batch_set_invalid';
  end if;

  v_boundary := public.xrpl_drain_r5_checkpoint_boundary(
    'r5-halted-boundary-repair',
    p_repaired_at
  );
  ...
  if coalesce((v_boundary->>'drained')::boolean, false) is not true
    ...
    or v_boundary_before_index < 4135151
    or v_boundary_after_index < v_boundary_before_index
    or v_boundary_after_index > v_boundary_before_index + 1
    or v_boundary_after_index > 4135407 then
    raise exception 'r5_halted_boundary_repair_boundary_invalid';
  end if;
  ...
  if not found
    ...
    or v_watermark.ledger_index > 4135407
    ...
    raise exception 'r5_halted_boundary_repair_physical_watermark_invalid';
  end if;
  ...
  where work.profile_id = 'supabase-devnet'
    ...
    and work.start_ledger_index = 4135151
    and work.scanned_end_ledger_index = 4135151;

  if not found
    or v_diagnostic_hash <> v_diagnostic_physical_hash
    or v_diagnostic_work_id <> v_diagnostic_physical_work then
    raise exception 'r5_halted_boundary_repair_diagnostic_anchor_invalid';
  end if;
  ...
  if v_work_count <> 24
    ...
    raise exception 'r5_halted_boundary_repair_first_chunk_invalid';
  end if;
  ...
  if v_first_rows_digest !~ '^[a-f0-9]{64}$' then
    raise exception 'r5_halted_boundary_repair_first_rows_invalid';
  end if;

  v_first_accounting_digest := encode(extensions.digest(convert_to(
    jsonb_build_object(
      'schemaVersion', 2,
      'purpose', 'r5-halted-executor-batch-adopted-after-bounded-boundary-drain',
      'runId', p_run_id,
      'batchId', p_batch_id,
      'batchSequence', 87,
  ...
  if v_run.status <> 'running'
    ...
    raise exception 'r5_halted_boundary_repair_first_update_invalid';
  end if;

  v_remaining_ledger_count := v_watermark.ledger_index - 4135136;
  v_expected_remaining_batches := (v_remaining_ledger_count + 23) / 24;

  if v_remaining_ledger_count < 15
    or v_remaining_ledger_count > 271
    or v_expected_remaining_batches < 1
    or v_expected_remaining_batches > 12 then
    raise exception 'r5_halted_boundary_repair_remaining_range_invalid';
  end if;
  ...
  if coalesce((v_remaining_adoption->>'adopted')::boolean, false) is not true
    ...
    raise exception 'r5_halted_boundary_repair_remaining_adoption_invalid';
  end if;
  ...
  if v_run.status <> 'running'
    ...
    or v_run.completed_batches <> 87 + v_expected_remaining_batches
    or v_run.committed_ledgers
    ...
    raise exception 'r5_halted_boundary_repair_final_state_invalid';
  end if;

  insert into xrpl_r5_v1.halted_descendant_repairs (
    run_id, batch_id,
    source_diagnostic_run_id, source_failed_deploy_run_id,
  ...
  return jsonb_build_object(
    ...
    'sourceFailedDeployRunId', 30920985639,
    'recoveryWatermarkBefore', 4135112,
    'diagnosticPhysicalWatermark', 4135151,
...
revoke all on function public.xrpl_repair_r5_halted_batch_after_boundary_drain(
  text, text, timestamptz
) from public, anon, authenticated;

grant execute on function public.xrpl_repair_r5_halted_batch_after_boundary_drain(
  text, text, timestamptz
) to service_role;
...
begin
  if exists (select 1 from pg_roles where rolname = 'supabase_admin') then
    execute 'grant execute on function public.xrpl_repair_r5_halted_batch_after_boundary_drain(text, text, timestamptz) to supabase_admin';
  end if;
end;
$$;
