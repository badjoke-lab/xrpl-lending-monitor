...
declare
  ...
  v_bytes bigint;
begin
  select * into v_session from xrpl_steady_v1.sessions
    where session_id = 'r4c2d-steady-msflb8fo-5ebc5adc';
  select * into v_watermark from public.xrpl_phase_watermarks
  ...
  return jsonb_build_object(
    ...
    'activeWatermark', case when v_watermark is null then null else jsonb_build_object(
...
begin
  ...
  select * into v_auth from xrpl_qualification_archive_v1.steady_reclaim_authorizations
    where authorization_id = p_authorization_id for update;
  if not found or v_auth.schema_version <> 1 or v_auth.issue_number <> 1261
     or v_auth.approved_by <> 'badjoke-lab' or v_auth.expires_at <= clock_timestamp()
     or v_auth.used_at is not null then
    raise exception 'r4f_steady_reclaim_authorization_invalid_or_expired';
  end if;

  select * into v_session from xrpl_steady_v1.sessions
    where session_id = 'r4c2d-steady-msflb8fo-5ebc5adc' for share;
  if not found or v_session.status <> 'completed'
     or v_session.target_ticks <> 6 or v_session.batch_size <> 24
     or v_session.completed_ticks <> 6 or v_session.committed_ledgers <> 144
     or v_session.source_profile_id <> 'supabase-devnet'
     or v_session.target_profile_id <> 'supabase-devnet-steady-qualification'
     or v_session.network <> 'devnet' or v_session.epoch_id <> 'supabase-r4c2c-v1' then
  ...
  select count(*) into v_lease_count from xrpl_steady_v1.ticks
    where lease_owner is not null and lease_expires_at is not null;
  ...
  select count(*) into v_completed_ticks from xrpl_steady_v1.ticks
    where session_id = v_session.session_id and status = 'completed';
  if v_tick_count <> 6 or v_completed_ticks <> 6 then
    raise exception 'r4f_steady_reclaim_tick_evidence_invalid';
  end if;

  select * into v_wm_before from public.xrpl_phase_watermarks
    where profile_id = 'supabase-devnet' for share;
  ...
  v_active_before := jsonb_build_object(
    'ledgerIndex', v_wm_before.ledger_index,
    'ledgerHash', v_wm_before.ledger_hash,
    'workId', v_wm_before.work_id
  );
  ...
  insert into xrpl_qualification_archive_v1.steady_reclaims (
    archive_id, authorization_id, source_workflow_run_id, source_commit,
    source_artifact_id, source_artifact_digest, retained_session_id, retained_tick_count,
    reclaimed_schema, physical_bytes_before, physical_bytes_after, table_counts_before,
    active_watermark_before, active_watermark_after, evidence, evidence_digest, reclaimed_at
  ) values (
    v_archive_id, p_authorization_id, 30975277983,
    'd7e6eb86eb0e660dffd3ad5e54d2fd995ba8a54c', 8924984813,
    'sha256:76f4580d83c053dadfe8a707c7bf53b53d99d361fd12c12adefe76061a9dafa3',
    v_session.session_id, 6, 'xrpl_steady_v1', v_before, 0, v_counts,
    v_active_before, '{}'::jsonb, v_evidence, public.xrpl_transfer_json_digest(v_evidence), clock_timestamp()
  );
  ...
  select count(*) into v_rows_after from (
    select 1 from xrpl_steady_v1.sessions limit 1
    union all select 1 from xrpl_steady_v1.ticks limit 1
    union all select 1 from xrpl_steady_v1.works limit 1
    union all select 1 from xrpl_steady_v1.messages limit 1
    union all select 1 from xrpl_steady_v1.successors limit 1
    union all select 1 from xrpl_steady_v1.payload_chunks limit 1
    union all select 1 from xrpl_steady_v1.reference_rows limit 1
    union all select 1 from xrpl_steady_v1.commit_chunks limit 1
  ) q;
  ...
  select * into v_wm_after from public.xrpl_phase_watermarks
    where profile_id = 'supabase-devnet' for share;
  if not found or v_wm_after.ledger_index is distinct from v_wm_before.ledger_index
     or v_wm_after.ledger_hash is distinct from v_wm_before.ledger_hash
     or v_wm_after.work_id is distinct from v_wm_before.work_id then
    raise exception 'r4f_steady_reclaim_active_watermark_changed';
  end if;

  v_active_after := jsonb_build_object(
    'ledgerIndex', v_wm_after.ledger_index,
    'ledgerHash', v_wm_after.ledger_hash,
    'workId', v_wm_after.work_id
  );
  v_final_evidence := v_evidence || jsonb_build_object(
    'physicalBytesAfter', v_after,
  ...
  set physical_bytes_after = v_after,
