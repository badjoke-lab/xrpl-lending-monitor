...
function parseFence(value: unknown, code: Code): Fence {
  ...
  if (
    raw.schemaVersion !== 1
    || raw.network !== 'devnet'
    || raw.epochId !== EPOCH_ID
    || raw.baseIdentity !== BASE_IDENTITY
  ) {
    throw new ReaderError(code, 'fence source identity is invalid')
  }
  const ledgerIndex = integer(raw.ledgerIndex, code, 'ledgerIndex')
  if (ledgerIndex !== 2_776_760) throw new ReaderError(code, 'fence ledger is invalid')
  return {
    schemaVersion: 1,
    network: 'devnet',
    epochId: EPOCH_ID,
...
function parseCursorQuery(value: unknown): Query {
...
async function decodeCursor(value: unknown): Promise<Cursor> {
  ...
  if (bytes.byteLength > MAX_CURSOR_BYTES || await digest(bytes) !== parts[2]) {
  ...
  return {
    ...
    query: parseCursorQuery(raw.query),
...
function parseRow(value: unknown, at: Fence): Row {
  ...
  const sourceLedgerIndex = integer(
    raw.sourceLedgerIndex,
    'integrity_failure',
    'sourceLedgerIndex',
  )
  if (sourceLedgerIndex !== at.ledgerIndex) {
    throw new ReaderError('integrity_failure', 'row is outside the fixed witness fence')
  }
  ...
  const relationships = [...new Set(raw.relationshipIds as string[])].sort((left, right) =>
    left.localeCompare(right),
  )
  if (
    relationships.some((entry) => !entry.trim())
    || canonicalPortableJson(relationships) !== canonicalPortableJson(raw.relationshipIds)
  ) {
    throw new ReaderError('integrity_failure', 'relationshipIds are not canonical')
  }
  const valueJson = raw.valueJson === null
    ? null
    : text(raw.valueJson, 'integrity_failure', 'valueJson')
  if (valueJson !== null) {
    let parsed: unknown
    try {
  ...
  const workId = text(raw.workId, 'integrity_failure', 'workId')
  if (workId !== at.workId) throw new ReaderError('integrity_failure', 'row work does not match fence')
  return {
    workId,
    semanticClass,
    ...
    objectId:
      raw.objectId === null ? null : text(raw.objectId, 'integrity_failure', 'objectId'),
...
async function rpc(
  ...
  let result: Response
  try {
    result = await fetch(`${url}/rest/v1/rpc/xrpl_read_multichunk_witness_page`, {
      method: 'POST',
    ...
    throw new ReaderError(
      'unavailable',
      `reader RPC failed: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
...
async function execute(body: Json, url: string, key: string): Promise<Json> {
  ...
  if (kind === 'fence') {
    ...
    return {
      schemaVersion: 1,
      source: source(),
      fence: parseFence(result.fence, 'integrity_failure'),
    }
  }
  ...
  if (kind === 'exact') {
    exactKeys(
      body,
      ['kind', 'sourceId', 'semanticClass', 'canonicalKey'],
      'invalid_query',
      'exact request',
    )
    const result = await rpc(
    ...
    if (!Array.isArray(result.rows)) {
      throw new ReaderError('integrity_failure', 'exact rows are unavailable')
    }
    return {
      schemaVersion: 1,
      source: source(),
  ...
  if (!['asc', 'desc'].includes(String(body.order))) {
  ...
  if (requested.kind === 'semantic' && requested.semanticClass === null) {
    throw new ReaderError('invalid_query', 'semantic page requires a class')
  }
  if (
  ...
  if (requested.kind === 'relationship' && requested.relationshipId === null) {
    throw new ReaderError('invalid_query', 'relationship page requires an identity')
  }
  ...
  if (body.cursor !== null) {
    ...
    if (canonicalPortableJson(cursor.query) !== canonicalPortableJson(requested)) {
...
function status(error: ReaderError): number {
  if (error.code === 'invalid_query' || error.code === 'invalid_cursor') return 400
  if (error.code === 'stale_cursor') return 409
...
Deno.serve(async (request) => {
  try {
    ...
    return response(await execute(body, env('SUPABASE_URL'), serviceKey()))
  } catch (error) {
    if (error instanceof ReaderError) {
      return response({ code: error.code, error: error.message }, status(error))
    }
    return response(
      { code: 'unavailable', error: error instanceof Error ? error.message : String(error) },
      503,
    )
  }
})
